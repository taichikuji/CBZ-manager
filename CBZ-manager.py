...
def extract_cbz_to_temp(cbz_files: Dict[Tuple[str, int, int], List[str]], temp_dir: Path) -> Dict[Tuple[str, int], Path]:
    extracted = {}
...
def extract_info(filename: str, manual_title: Optional[str] = None) -> Tuple[str, int, int]:
...
def process_files(files: List[str], manual_title: Optional[str] = None) -> Dict[Tuple[str, int, int], List[str]]:
...
def combine_volumes(extracted: Dict[Tuple[str, int], Path], temp_dir: Path) -> Tuple[Path, str]:
...
def main():
    ...
    parser.add_argument('--title', type=str, help='Manually set the title for sorting', default=None)
